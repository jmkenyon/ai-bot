...
export function syntaxCheckRule(input: string): string {
  ...
  for (let i = 0; i < rule.length; i++) {
    ...
    if (ch === "(") { parenDepth++; parenStack.push(i); }
    if (ch === ")") {
      parenDepth--;
      parenStack.pop();
      if (parenDepth < 0) issues.push(`Unexpected ')' at position ${i}`);
    }
    if (ch === "[") { bracketDepth++; bracketStack.push(i); }
    if (ch === "]") {
      bracketDepth--;
  ...
  if (parenDepth > 0) {
    issues.push(`Unbalanced parentheses: ${parenDepth} unclosed '(' — last opened near position ${parenStack[parenStack.length - 1]}`);
  }
  if (parenDepth < 0) {
    issues.push(`Unbalanced parentheses: ${Math.abs(parenDepth)} extra ')'`);
  }
  if (bracketDepth > 0) {
    issues.push(`Unbalanced brackets: ${bracketDepth} unclosed '[' — near position ${bracketStack[bracketStack.length - 1]}`);
  }
  ...
  if (!targetMatch) {
    ...
    if (missingBracket) {
      issues.push(`Missing ']' after event codes '${missingBracket[2]}' — should be ${missingBracket[1]}[${missingBracket[2]}]=`);
    } else {
      const missingEquals = rule.match(/^(#\d+|\d+:\d+)\[([A-Z]*)\][^=]/);
      if (missingEquals) {
        issues.push(`Missing '=' after ${missingEquals[1]}[${missingEquals[2]}]`);
      } else {
  ...
  // ── 3. Helpers ──────────────────────────────────────────────────
  function extractFuncBody(source: string, funcName: string): { body: string; startIdx: number; endIdx: number } | null {
  ...
  if (conditionalInfo) {
    ...
    if (condArgs.length !== 3) {
      const argSummary = condArgs.map((a, i) => `  arg${i + 1}: ${a.length > 50 ? a.substring(0, 50) + "..." : a}`).join("\n");
      issues.push(`Conditional() requires exactly 3 arguments (condition, trueBranch, falseBranch) — found ${condArgs.length}:\n${argSummary}`);
    } else {
      const actionPattern = /^(Replace|Ignore|Remove|Constant|Basic|Conditional)\(/;
      if (!actionPattern.test(condArgs[1])) {
        issues.push(`Conditional() arg 2 (trueBranch) should be an action — found: ${condArgs[1].substring(0, 40)}`);
      }
      if (!actionPattern.test(condArgs[2])) {
        issues.push(`Conditional() arg 3 (falseBranch) should be an action — found: ${condArgs[2].substring(0, 40)}`);
      }
    }
  }
  ...
  for (const funcName of ["And", "Or"]) {
    ...
    if (funcInfo) {
      ...
      for (const arg of funcArgs) {
        ...
        if (actionMatch) {
          issues.push(`'${actionMatch[1]}()' is an action, not a condition — must NOT be inside ${funcName}(). ${funcName}() is likely missing its closing ')' before '${actionMatch[1]}()'.`);
        }
      }
    }
  }
  ...
  while ((fidMatch = fidPattern.exec(withoutStrings)) !== null) {
    const preceding = withoutStrings.substring(Math.max(0, fidMatch.index - 1), fidMatch.index);
    if (preceding !== "#") {
      issues.push(`FID ${fidMatch[1]} may be missing a type suffix (e.g. ${fidMatch[1]}:6)`);
    }
  }

  // ── 7. Return ───────────────────────────────────────────────────
  if (issues.length === 0) {
    const funcsUsed = [...new Set([...rule.matchAll(/\b([A-Z][a-zA-Z]+)\(/g)].map((m) => m[1]))];
